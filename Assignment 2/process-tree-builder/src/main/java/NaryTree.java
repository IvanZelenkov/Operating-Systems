...
public class NaryTree<T> {
    ...
    /**
     * Check if there is such a node in a tree.
     * @param node Node to find.
     * @param keyNode property is used to find node.
    ...
    /**
     * Find node based on an keyNode value.
     * @param node Node to find.
     * @param keyNode property is used to find node.
     * @return node containing its data.
     */
    public Node<T> findNode(Node<T> node, T keyNode) {
        ...
        if (node.getPid().equals(keyNode))
            ...
            for (Node<T> child : node.getChildren())
                if ((currentNode = findNode(child, keyNode)) != null)
                    return currentNode;
        }
        return null;
    }
